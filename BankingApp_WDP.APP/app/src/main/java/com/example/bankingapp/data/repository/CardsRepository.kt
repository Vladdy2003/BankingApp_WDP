...
class CardsRepository {
    ...
    suspend fun getCard(id: String): CardResponse =
        RetrofitClient.cardsApi.getCard(id)
    ...
    suspend fun updateLimits(id: String, request: UpdateCardLimitsRequest) {
        val response = RetrofitClient.cardsApi.updateLimits(id, request)
        if (!response.isSuccessful) throw retrofit2.HttpException(response)
    }

    suspend fun deleteCard(id: String) {
        val response = RetrofitClient.cardsApi.deleteCard(id)
        if (!response.isSuccessful) throw retrofit2.HttpException(response)
    }
}
